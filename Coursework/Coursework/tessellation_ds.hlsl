//Jamie Haddow tessellation domain Shader
//COMMENTING DONE
...
OutputType main(ConstantOutputType input, float2 uvwCoord : SV_DomainLocation, const OutputPatch<InputType, 4> patch)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        output.pLightViewsPos[i] = mul(float4(vertexPosition, 1.0f), worldMatrix);
        ...
        output.pLightViewsPos2[i] = mul(float4(vertexPosition, 1.0f), worldMatrix);
